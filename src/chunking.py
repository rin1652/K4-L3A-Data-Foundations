...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Split sentences preserving text, splitting on boundary following punctuation
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for piece in splits:
            if len(piece) > self.chunk_size:
                if good_splits:
                    final_chunks.extend(self._merge(good_splits, sep))
                    good_splits = []
    ...
    def _merge(self, pieces: list[str], sep: str) -> list[str]:
        ...
        for p in pieces:
            if not p:
                continue
            add_len = len(p) + (len(sep) if current else 0)
